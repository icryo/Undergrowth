...
VOID DumpListOfExport(VOID *lib, CHAR *name, HANDLE hDll);
...
VOID DumpListOfExport(VOID *lib, CHAR *name, HANDLE hDll) {
    ...
    for(dwIter; dwIter < dwFunctionsCount - 1; dwIter++) {
        ...
        if(strcmp(current, name) == 0) {
            ...
            printf("%s syscall is %02x%02x\n", name, high, id);
            printf("We are going to patch %s at %p\n", name, toPatchAddr);
...
int main (int argc, char **argv) {
    ...
    printf("data is at %p\n", data);
    DumpListOfExport(data, "NtIsSystemResumeAutomatic", hDll);
    DumpListOfExport(data, "NtOpenProcess", hDll);
